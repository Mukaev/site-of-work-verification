...
exports.getLogin = (req, res, next) => {
    res.render('login', { isAutenticated: false, role: 'u' });
};
...
getCompare = (req, res, password, user) => {
    ...
    bcrypt.compare(password, user[0].password)
        .then((matched) => {
            ...
            return res.render('main',{isAutenticated:false,role:'u'});
    ...
    Professor.findAll({
        ...
        if (!user || user.length == 0) {
            ...
            Student.findAll({
                ...
                if (!user || user.length == 0) return res.redirect('/login');
...
exports.getSignup = (req, res, next) => {
    res.render('signup', { isAutenticated: false, role: 'u' });
};
// процесс регистрации
